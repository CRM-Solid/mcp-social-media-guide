...
To try the first row: the package is on [npm](https://www.npmjs.com/package/@crmsolid/mcp-server), the source is on [GitHub](https://github.com/CRM-Solid/crmsolid-mcp), and the tool reference is at [docs.crmsolid.com/integrations/mcp/](https://docs.crmsolid.com/integrations/mcp/). Tutorial 01 uses it for the worked example because a concrete server beats a hypothetical one. The concepts transfer to every other option in the table.
